...
model = torch.load('best_network4_3.12.model')
...
with torch.no_grad():
    ...
    for batch_i, data in batching:
        ...
        nss_fix.append(NSS(pred_fix[:, :, 0, :, :], points_fix))
        nss_sp.append(NSS(pred_sp[:, :, 0, :, :], points_sp))
...
nss_fix = np.nan_to_num(np.array(nss_fix))
nss_sp = np.nan_to_num(np.array(nss_sp))

print(f'NSS Fix: {np.mean(nss_fix):2E}  Std: {np.std(nss_fix):2E}  Min: {np.min(nss_fix):2E}  Max: {np.max(nss_fix):2E}')
print(f'NSS SP: {np.mean(nss_sp):2E}  Std: {np.std(nss_sp):2E}  Min: {np.min(nss_sp):2E}  Max: {np.max(nss_sp):2E}')
