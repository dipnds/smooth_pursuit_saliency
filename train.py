...
eval_loader = DataLoader(ev_set, batch_size=batch_size, shuffle=True, num_workers=10)
train_loader = DataLoader(tr_set, batch_size=batch_size, shuffle=True, num_workers=10)
...
def train(model, epochNum):
    ...
    for batch_i, data in train_batching:
        ...
        # sample = ip[0][0].squeeze()
        # fig, ax = plt.subplots(1,3)
        # ax[0].imshow(data['op'][0, 0, 0, :, :], vmin=0, vmax=1)
        # ax[1].imshow(data['op'][0, 0, 1, :, :], vmin=0, vmax=1)
        # ax[2].imshow(data['op'][0, 0, 2, :, :], vmin=0, vmax=1)
        # plt.show()
        # exit()
        ...
        if (batch_i+1) % log_nth == 0:
            train_batching.set_description(f'Train E: {epoch+1}, B: {batch_i+1}, L:{np.mean(tr_loss_sp)+np.mean(tr_loss_fix):.2E}')
...
def evaluate(model, epoch, best_eval, scheduler):
    ...
    with torch.no_grad():
        ...
        loss = np.mean(ev_loss_fix) + np.mean(ev_loss_sp)
        ipImg = ip[0,0,:,:,:]
        ipImg = (ipImg - ipImg.min()) / (ipImg.max() - ipImg.min())
